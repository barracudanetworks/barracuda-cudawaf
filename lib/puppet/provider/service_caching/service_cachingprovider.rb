...
Puppet::Type.type(:service_caching).provide(:service_cachingprovider) do
  ...
  def exists?
    ...
    #call get service_caching
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_caching_instance.services_web_application_name_caching_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    ...
    Puppet.debug("Calling instances method of service_cachingprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
