...
require_relative '../../../puppet_x/modules/website_profile_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'
Puppet::Type.type(:service_website_profile).provide(:service_website_profileprovider) do
  ...
  def exists?
    ...
    #call get service_website_profile
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers= service_websiteprof_instance.services_web_application_name_website_profile_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_website_profileprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_website_profile from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_website_profile response:    #{data}"
        unless data == '{}'
      if status_code == 200
