...
require_relative '../../../puppet_x/modules/ssl_client_authentication_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'
...
Puppet::Type.type(:service_ssl_client_authentication).provide(:service_ssl_client_authenticationprovider) do
  ...
  def exists?
    ...
    #call get service_ssl_client_authentication
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_sslauth_instance.services_web_application_name_ssl_client_authentication_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_ssl_client_authenticationprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_ssl_client_authentication from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_ssl_client_authentication response:    #{data}"
    unless data == '{}'
      if status_code == 200
