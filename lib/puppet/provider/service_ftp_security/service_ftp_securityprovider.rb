...
Puppet::Type.type(:service_ftp_security).provide(:service_ftp_securityprovider) do
  ...
  def exists?
    ...
    #call get service_ftp_security
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_ftpsecurity_instance.services_web_application_name_ftp_security_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_ftp_securityprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_ftp_security from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_ftp_security response:    #{data}"
    unless data == '{}'
      if status_code == 200
