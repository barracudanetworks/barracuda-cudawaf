...
Puppet::Type.type(:service_sensitive_parameter_names).provide(:service_sensitive_parameter_namesprovider) do
  ...
  def exists?
    ...
    #call get service_sensitive_parameter_names
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_sensitiveparams_instance.services_web_application_name_sensitive_parameter_names_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    ...
    Puppet.debug("Calling instances method of service_sensitive_parameter_namesprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
