...
require_relative '../../../puppet_x/modules/ssl_security_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'
...
Puppet::Type.type(:service_ssl_security).provide(:service_ssl_securityprovider) do
  ...
  def exists?
    ...
    #call get service_ssl_security
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_sslsec_instance.services_web_application_name_ssl_security_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_ssl_securityprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_ssl_security from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_ssl_security response:    #{data}"
        unless data == '{}'
      if status_code == 200
