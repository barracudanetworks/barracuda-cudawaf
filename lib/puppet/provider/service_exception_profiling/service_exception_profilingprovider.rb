...
require_relative '../../../puppet_x/modules/exception_profiling_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'
Puppet::Type.type(:service_exception_profiling).provide(:service_exception_profilingprovider) do
  ...
  def exists?
    ...
    #call get service_exception_profiling
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers= service_excep_instance.services_web_application_name_exception_profiling_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    ...
    Puppet.debug("Calling instances method of service_exception_profilingprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_exception_profiling from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_exception_profiling response:    #{data}"
    unless data == '{}'
      if status_code == 200
