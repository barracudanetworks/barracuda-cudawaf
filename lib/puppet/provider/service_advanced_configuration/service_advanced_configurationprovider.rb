...
Puppet::Type.type(:service_advanced_configuration).provide(:service_advanced_configurationprovider) do
  ...
  def exists?
    ...
    #call get service_advanced_configuration
    svcName=@resource[:name]
    Puppet.debug("WAF service  name in manifest:  #{svcName}")
    data,status_code,headers=service_adv_config_instance.services_web_application_name_advanced_configuration_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    ...
    Puppet.debug("Calling instances method of service_advanced_configurationprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_api=SwaggerClient::ServiceApi.new    
    # get all service_advanced_configuration from WAF
    data,status_code,headers = service_api.services_get(auth_header,{})
    Puppet.debug("WAF Get all service response:    #{data}")
...
=begin
    ...
    service_adv_config_instance=SwaggerClient::AdvancedConfigurationApi.new
    svcName=@resource[:name]
    data,status_code,headers= service_adv_config_instance.services_web_application_name_advanced_configuration_post(auth_header,svcName,message(resource),{})
    Puppet.debug("WAF service_advanced_configuration CREATE response:  #{data}")
    if status_code == 201
      @property_hash.clear
      return data
    else
      fail("Not able to create the service_advanced_configuration. Please check the service api parameters")
    end
=end
  end
