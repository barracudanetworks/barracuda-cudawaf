...
require_relative '../../../puppet_x/modules/server/application_layer_health_checks_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'

Puppet::Type.type(:server_application_layer_health_checks).provide(:server_application_layer_health_checksprovider) do
  Puppet.debug("Inside server_application_layer_health_checksprovider: ")
  mk_resource_methods

  def exists?
    Puppet.debug("Calling exists method of server_application_layer_health_checksprovider: ")
    ...
    app_layer_server_instance = SwaggerClient::ApplicationLayerHealthChecksApi.new
    #call get service
    ...
    if service_status_code === '200'
       # call get server
      data,status_code,headers= app_layer_server_instance.services_web_application_name_servers_web_server_name_application_layer_health_checks_get(auth_header,serviceName,serverName)
  ...
  def self.instances
    ...
    services.each do |service|
      svc = service
      Puppet.debug("Calling getInstances method of server_application_layer_health_checksprovider: ")
      serviceName=svc
      Puppet.debug("Service Name : #{serviceName}")
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      server_instance = SwaggerClient::ServerApi.new
      # get all server_application_layer_health_checks from WAF
      data,status_code,headers = server_instance.services_web_application_name_servers_get(auth_header,serviceName,{})
      Puppet.debug("WAF Get all server_application_layer_health_checks response:    #{data}")
  ...
  def self.getservices
     Puppet.debug("Calling getservices  method of server_application_layer_health_checksprovider: ")
     service_instances = []
     login_instance = Login.new
     auth_header = login_instance.get_auth_header
     service_instance = SwaggerClient::ServiceApi.new
  ...
  def self.prefetch(resources)
    Puppet.debug("Calling prefetch method of server_application_layer_health_checksprovider: ")
    servers = instances
  ...
  def flush
    Puppet.debug("Calling flush method of server_application_layer_health_checksprovider: ")
    if @property_hash != {}
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      server_instance = SwaggerClient::ServerApi.new
      ...
      serverName=@resource[:name]
      serviceName=@resource[:service_name]
      data,status_code,headers = app_layer_server_instance.services_web_application_name_servers_web_server_name_application_layer_health_checks_put(auth_header,serviceName,serverName,message(resource),{})
  ...
  def create
    Puppet.debug("Calling create method of server_application_layer_health_checksprovider: ")
  end

  def destroy
    Puppet.debug("Calling destroy method of server_application_layer_health_checksprovider: ")
  end
end
