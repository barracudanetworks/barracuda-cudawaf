...
Puppet::Type.type(:security_policy_cloaking).provide(:security_policy_cloakingprovider) do
 ...
 # this method will get security_policy_cloaking and return true or false 
  ...
  def exists?
    ...
    #call get security_policy_cloaking
    policyName=@resource[:name]
    Puppet.debug("WAF secuirty policy name in manifest:  #{policyName}")
    data,status_code,headers=security_policy_cloaking_instance.security_policies_policy_name_cloaking_get(auth_header,policyName)
    if status_code == 200
       true
