...
require_relative '../../../puppet_x/modules/connection_pooling_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'
...
Puppet::Type.type(:server_connection_pooling).provide(:server_connection_poolingprovider) do
  ...
  def exists?
    ...
    if service_status_code === '200'
       # call get server
      data,status_code,headers=con_pool_server_instance.services_web_application_name_servers_web_server_name_connection_pooling_get(auth_header,serviceName,serverName)
  ...
  def self.instances
    ...
    services.each do |service|
      svc = service
      Puppet.debug("Calling getInstances method of server_connection_poolingprovider: ")
      serviceName=svc
      Puppet.debug("Service Name : #{serviceName}")
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      server_instance = SwaggerClient::ServerApi.new
      # get all server_connection_pooling from WAF
      data,status_code,headers = server_instance.services_web_application_name_servers_get(auth_header,serviceName,{})
      Puppet.debug("WAF Get all server_connection_pooling response:    #{data}")
  ...
  def self.prefetch(resources)
    Puppet.debug("Calling prefetch method of server_connection_poolingprovider: ")
    servers = instances
