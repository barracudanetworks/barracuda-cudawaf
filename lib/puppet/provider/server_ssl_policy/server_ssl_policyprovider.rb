...
Puppet::Type.type(:server_ssl_policy).provide(:server_ssl_policyprovider) do
  ...
  def exists?
    ...
    if service_status_code === '200'
       # call get server
      data,status_code,headers=ssl_policy_server_instance.services_web_application_name_servers_web_server_name_ssl_policy_get(auth_header,serviceName,serverName)
  ...
  def self.instances
    ...
    services.each do |service|
      svc = service
      Puppet.debug("Calling getInstances method of server_ssl_policyprovider: ")
      serviceName=svc
      Puppet.debug("Service Name : #{serviceName}")
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      server_instance = SwaggerClient::ServerApi.new
      # get all server_ssl_policy from WAF
      data,status_code,headers = server_instance.services_web_application_name_servers_get(auth_header,serviceName,{})
      Puppet.debug("WAF Get all server_ssl_policy response:    #{data}")
