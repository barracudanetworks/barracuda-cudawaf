...
require_relative '../../../puppet_x/modules/security_policy/security_policy_url_protection_api.rb'
require 'json'
require 'base64'
require 'logger'
require 'uri'

Puppet::Type.type(:security_policy_url_protection).provide(:security_policy_url_protectionprovider) do
  Puppet.debug("Inside security_policy_url_protectionprovider: ")
  mk_resource_methods

 # this method will get security_policy_url_protection and return true or false 
 ...
 #this method get all secuirty policys from WAF system and builds the instances array 
  ...
  def self.instances
    ...
    Puppet.debug("Calling instances method of security_policy_url_protectionprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    security_policy_instance = SwaggerClient::SecurityPolicyApi.new
    # get all security_policy_url_protection from WAF
    data,status_code,headers = security_policy_instance.security_policies_get(auth_header,{})
    Puppet.debug("WAF Get all security_policy_url_protection response:    #{data}")
    unless data == '{}'
      if status_code == 200
 ...
 # this method compares the name attribute from instances and resources. If it matches then sets the provider
  def self.prefetch(resources)
    Puppet.debug("Calling prefetch method of security_policy_url_protectionprovider: ")
 ...
 # this method does a put call to wafsecurity_policy_url_protection. This will be triggered with ensure is present and exists method return true.
  def flush
    Puppet.debug("Calling flush method of security_policy_url_protectionprovider: ")
    if @property_hash != {}
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      security_policy_url_protect_instance = SwaggerClient::SecurityPolicyUrlProtectionApi.new
      policyName=@resource[:name]
      data,status_code,headers= security_policy_url_protect_instance.security_policies_policy_name_url_protection_put(auth_header,policyName,message(resource),{})
 ...
 # this method does a POST call to create a security_policy_url_protection in WAF.this method will be called if the ensure is present and exists method return false 
  def create
    Puppet.debug("Calling create method of security_policy_url_protectionprovider: ")
  end

# this method will call the delete api of a WAF security_policy_url_protection 
  def destroy
