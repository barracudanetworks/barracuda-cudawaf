...
Puppet::Type.type(:server_load_balancing).provide(:server_load_balancingprovider) do
  ...
  def exists?
    ...
    if service_status_code === '200'
       # call get server
      data,status_code,headers= load_bal_api_instance.services_web_application_name_servers_web_server_name_load_balancing_get(auth_header,serviceName,serverName)
  ...
  def self.instances
    ...
    services.each do |service|
      svc = service
      Puppet.debug("Calling getInstances method of server_load_balancingprovider: ")
      serviceName=svc
      Puppet.debug("Service Name : #{serviceName}")
      login_instance = Login.new
      auth_header = login_instance.get_auth_header
      server_instance = SwaggerClient::ServerApi.new
      # get all server_load_balancing from WAF
      data,status_code,headers = server_instance.services_web_application_name_servers_get(auth_header,serviceName,{})
      Puppet.debug("WAF Get all server_load_balancing response:    #{data}")
  ...
  def self.prefetch(resources)
    Puppet.debug("Calling prefetch method of server_load_balancingprovider: ")
    servers = instances
