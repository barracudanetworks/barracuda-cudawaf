...
def self.instances()
 ...
 services.each do |service|
   ...
   svc = service
   Puppet.debug("Calling getInstances method of url encryption provider: ")
   serviceName=svc
   Puppet.debug("Service Name : #{serviceName}")
   login_instance = Login.new
   auth_header = login_instance.get_auth_header
   service_urlencrp_instance = SwaggerClient::UrlEncryptionApi.new
   data,status_code,headers = service_urlencrp_instance.services_web_application_name_url_encryption_get(auth_header,serviceName,{})
