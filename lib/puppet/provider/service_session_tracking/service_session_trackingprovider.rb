...
Puppet::Type.type(:service_session_tracking).provide(:service_session_trackingprovider) do
  ...
  def exists?
    ...
    #call get service_session_tracking
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_instance.services_web_application_name_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_session_trackingprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
    service_instance = SwaggerClient::ServiceApi.new
    # get all service_session_tracking from WAF
    data,status_code,headers = service_instance.services_get(auth_header,{})
    Puppet.debug("WAF Get all service_session_tracking response:    #{data}"
    unless data == '{}'
      if status_code == 200
