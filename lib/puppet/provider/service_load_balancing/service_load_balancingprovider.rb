...
Puppet::Type.type(:service_load_balancing).provide(:service_load_balancingprovider) do
  ...
  def exists?
    ...
    #call get service_load_balancing
    svcName=@resource[:name]
    Puppet.debug("WAF service name in manifest:  #{svcName}")
    data,status_code,headers=service_loadbalance_instance.services_web_application_name_load_balancing_get(auth_header,svcName)
    if status_code == 200
       true
  ...
  def self.instances
    Puppet.debug("Calling instances method of service_load_balancingprovider: ")
    instances = []
    login_instance = Login.new
    auth_header = login_instance.get_auth_header
